...
EDF_ROOT = ROOT / "v3.0.1" / "edf"
DATA_DIR = ROOT / "data-annotated"
ARTIFACTS = ["chew", "elec", "elpp", "eyem", "musc", "shiv"]
SEIZURES = ["cpsz", "fnsz", "gnsz", "tcsz"]
LABELS = [*ARTIFACTS, "seiz", *SEIZURES]
COLORS = dict(zip(LABELS, ["#8e44ad", "#e67e22", "#16a085", "#3498db", "#e74c3c", "#2ecc71", "#c0392b", "#d35400", "#f1c40f", "#1abc9c", "#9b59b6"]))


def rows(path: Path, start: float, stop: float, allowed: list[str]) -> list[dict]:
    if not path.exists(): return []
    lines = [x for x in path.read_text(errors="ignore").splitlines() if x and not x.startswith("#")]
    out = []
    for r in csv.DictReader(StringIO("\n".join(lines))):
        a, b = float(r["start_time"]), float(r["stop_time"])
        if a >= stop or b <= start: continue
        for label in r.get("label", "").split("_"):
            if label in allowed: out.append({"start": max(a, start), "stop": min(b, stop), "label": label})
    return out


def annotations(edf: Path, start: float, stop: float) -> list[dict]:
    anns = rows(edf.with_suffix(".csv"), start, stop, ARTIFACTS) + rows(edf.with_name(edf.stem + "_seiz.csv"), start, stop, SEIZURES)
    seen = set(); out = []
    for x in anns:
...
def main() -> int:
    p = argparse.ArgumentParser(description="Render annotated TUAR EDF previews.")
