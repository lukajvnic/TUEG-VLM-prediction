...
def labels_for(edf: Path, start: float, stop: float) -> set[str]:
    found = set()
    for ann, allowed in [(edf.with_suffix(".csv"), ARTIFACTS), (edf.with_name(edf.stem + "_seiz.csv"), SEIZURES)]:
        ...
        for row in csv.DictReader(StringIO("\n".join(lines))):
            if float(row["start_time"]) >= stop or float(row["stop_time"]) <= start: continue
            for label in row.get("label", "").split("_"):
                if label in allowed: found.add(label)
    if found & set(SEIZURES): found.add("seiz")
    return found
...
def main() -> int:
    p = argparse.ArgumentParser(description="Render TUAR EDFs to PNGs and write artifact plus optional seizure labels.")
